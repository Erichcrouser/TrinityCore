...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    if (!AHBSeller)
    {
        if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHSeller: Disabled");
        return;
    }
    ...
    if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHSeller: Adding %u Auctions", items);
    ...
    case 7:
        ...
        break;
    default:
        if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHSeller: GetAHID() - Default switch reached");
        AuctioneerGUID = 23442; //default to neutral 7
        break;
    }

    if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHSeller: Current Auctineer GUID is %u", AuctioneerGUID);
...
*/
    ...
    if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHSeller: %u items", items);

    // only insert a few at a time, so as not to peg the processor
    for (uint32 cnt = 1; cnt <= items; cnt++)
    {
    if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHSeller: %u count", cnt);
        uint32 itemID = 0;
        uint32 itemColor = 99;
        ...
        while (itemID == 0 && loopbreaker <= 50)
        {
            ...
            default:
                {
                    if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHSeller: itemID Switch - Default Reached");
                    break;
                }
            }
            ...
            if (itemID == 0)
            {
                if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHSeller: Item::CreateItem() - ItemID is 0");
                continue;
            }
            ...
            if (prototype == NULL)
            {
                if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHSeller: Huh?!?! prototype == NULL");
                continue;
            }
            ...
            if (item == NULL)
            {
                if (debug_Out) (LOG_FILTER_NETWORKIO, "AHSeller: Item::CreateItem() returned NULL");
                break;
            }
            ...
            else
            {
                ...
                if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHBuyer: Quality %u not Supported", prototype->Quality);
            ...
            auctionEntry->owner = AHBplayerGUID;
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    if (!AHBBuyer)
    {
        if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Disabled");
        return;
    }
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        if (!pItem)
        {
            if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHBuyer: Item %u doesn't exist, perhaps bought already?", auction->itemGUIDLow);
            continue;
        }
        ...
        if (debug_Out)
        {
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "-------------------------------------------------");
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Info for Auction #%u:", auction->Id);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: AuctionHouse: %u", auction->GetHouseId());
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Auctioneer: %u", auction->auctioneer);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Owner: %u", auction->owner);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Bidder: %u", auction->bidder);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Starting Bid: %u", auction->startbid);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Current Bid: %u", currentprice);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Buyout: %u", auction->buyout);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Deposit: %u", auction->deposit);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Expire Time: %u", auction->expire_time);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Bid Rate: %f", bidrate);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Bid Max: %f", bidMax);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Bid Value: %f", bidvalue);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Bid Price: %u", bidprice);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Item GUID: %u", auction->itemGUIDLow);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Item Template: %u", auction->itemEntry);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Item Info:");
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Item ID: %u", prototype->ItemId);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Buy Price: %u", prototype->BuyPrice);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Sell Price: %u", prototype->SellPrice);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Bonding: %u", prototype->Bonding);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Quality: %u", prototype->Quality);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AHBuyer: Item Level: %u", prototype->ItemLevel);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AAHBuyer: Ammo Type: %u", prototype->AmmoType);
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "-------------------------------------------------");
        }
        ...
        if ((bidprice < auction->buyout) || (auction->buyout == 0))
        {
            ...
            if (auction->bidder > 0)
            {
                if (auction->bidder == AHBplayerGUID)
                {
        ...
        else
        {
            SQLTransaction trans = CharacterDatabase.BeginTransaction();
            //buyout
            if ((auction->bidder) && (AHBplayerGUID != auction->bidder))
            {
...
void AuctionHouseBot::Initialize()
{
    ...
    DisableBeta_PTR_Unused = sConfigMgr->GetBoolDefault("AuctionHouseBot.DisableBeta_PTR_Unused", false);
    ...
    if ((AHBplayerAccount != 0) || (AHBplayerGUID != 0))
    {
        ...
        if (!result)
       {
           TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AuctionHouseBot: The account/GUID-information set for your AHBot is incorrect (account: %u guid: %u)", AHBplayerAccount, AHBplayerGUID);
           return;
        }
    }
    ...
    if (AHBSeller)
    {
        ...
        {
            if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: \"%s\" failed", npcQuery);
        }
        ...
        {
            if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: \"%s\" failed", lootQuery);
        }
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (PTR/Beta/Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Permanent Enchant Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Conjured Consumable)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Gem)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Money)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (MoneyLoot)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Lootable Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Quest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Has a Duration)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (BOP or BQI and Required Level is less than Item Level)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Warrior Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Paladin Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Hunter Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Rogue Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Priest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (DK Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Shaman Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Mage Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Warlock Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Druid Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Item Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
           ...
           // Disable Trade Goods below level X
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Trade Good %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
           ...
           // Disable Trade Goods above level X
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Trade Good %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            // {
            //    if (debug_Out_Filters) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", itr->second.ItemId, itr->second.RequiredSkillRank);
            //    continue;
            // }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AuctionHouseBot: No items");
            AHBSeller = 0;
        }

        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot:");
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u grey trade goods", greyTradeGoodsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u white trade goods", whiteTradeGoodsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u green trade goods", greenTradeGoodsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u blue trade goods", blueTradeGoodsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u purple trade goods", purpleTradeGoodsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u orange trade goods", orangeTradeGoodsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u yellow trade goods", yellowTradeGoodsBin.size());   
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u grey items", greyItemsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u white items", whiteItemsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u green items", greenItemsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u blue items", blueItemsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u purple items", purpleItemsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u orange items", orangeItemsBin.size());
        TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "loaded %u yellow items", yellowItemsBin.size()); 
    }
    TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "AuctionHouseBot and AuctionHouseBuyer have been loaded.");
}
...
void AuctionHouseBot::IncrementItemCounts(AuctionEntry* ah)
{
    ...
    if (!pItem)
    {
        if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHBot: Item %u doesn't exist, perhaps bought already?", ah->itemGUIDLow);
        return;
    }
    ...
    FactionTemplateEntry const* uEntry = sFactionTemplateStore.LookupEntry(ah->GetHouseFaction());
    if (!uEntry)
    {
        if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHBot: %u returned as House Faction. Neutral", ah->GetHouseFaction());
        config = &NeutralConfig;
    }
    else if (uEntry->ourMask & FACTION_MASK_ALLIANCE)
    {
        if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHBot: %u returned as House Faction. Alliance", ah->GetHouseFaction());
        config = &AllianceConfig;
    }
    else if (uEntry->ourMask & FACTION_MASK_HORDE)
    {
        if (debug_Out) TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "AHBot: %u returned as House Faction. Horde", ah->GetHouseFaction());
        config = &HordeConfig;
    }
    else
    {
...
{
    if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Start Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetString().c_str());
    if (AHBSeller) 
    {
        ...
        if (debug_Out)
        {
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minItems                = %u", config->GetMinItems());
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxItems                = %u", config->GetMaxItems());
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentGreyTradeGoods   = %u", config->GetPercentages(AHB_GREY_TG));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentWhiteTradeGoods  = %u", config->GetPercentages(AHB_WHITE_TG));
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentGreenTradeGoods  = %u", config->GetPercentages(AHB_GREEN_TG));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentBlueTradeGoods   = %u", config->GetPercentages(AHB_BLUE_TG));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentPurpleTradeGoods = %u", config->GetPercentages(AHB_PURPLE_TG));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentOrangeTradeGoods = %u", config->GetPercentages(AHB_ORANGE_TG));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentYellowTradeGoods = %u", config->GetPercentages(AHB_YELLOW_TG));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentGreyItems        = %u", config->GetPercentages(AHB_GREY_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentWhiteItems       = %u", config->GetPercentages(AHB_WHITE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentGreenItems       = %u", config->GetPercentages(AHB_GREEN_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentBlueItems        = %u", config->GetPercentages(AHB_BLUE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentPurpleItems      = %u", config->GetPercentages(AHB_PURPLE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentOrangeItems      = %u", config->GetPercentages(AHB_ORANGE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "percentYellowItems      = %u", config->GetPercentages(AHB_YELLOW_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPriceGrey            = %u", config->GetMinPrice(AHB_GREY));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPriceGrey            = %u", config->GetMaxPrice(AHB_GREY));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPriceWhite           = %u", config->GetMinPrice(AHB_WHITE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPriceWhite           = %u", config->GetMaxPrice(AHB_WHITE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPriceGreen           = %u", config->GetMinPrice(AHB_GREEN));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPriceGreen           = %u", config->GetMaxPrice(AHB_GREEN));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPriceBlue            = %u", config->GetMinPrice(AHB_BLUE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPriceBlue            = %u", config->GetMaxPrice(AHB_BLUE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPricePurple          = %u", config->GetMinPrice(AHB_PURPLE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPricePurple          = %u", config->GetMaxPrice(AHB_PURPLE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPriceOrange          = %u", config->GetMinPrice(AHB_ORANGE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPriceOrange          = %u", config->GetMaxPrice(AHB_ORANGE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minPriceYellow          = %u", config->GetMinPrice(AHB_YELLOW));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxPriceYellow          = %u", config->GetMaxPrice(AHB_YELLOW));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPriceGrey         = %u", config->GetMinBidPrice(AHB_GREY));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPriceGrey         = %u", config->GetMaxBidPrice(AHB_GREY));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPriceWhite        = %u", config->GetMinBidPrice(AHB_WHITE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPriceWhite        = %u", config->GetMaxBidPrice(AHB_WHITE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPriceGreen        = %u", config->GetMinBidPrice(AHB_GREEN));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPriceGreen        = %u", config->GetMaxBidPrice(AHB_GREEN));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPriceBlue         = %u", config->GetMaxBidPrice(AHB_BLUE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPricePurple       = %u", config->GetMinBidPrice(AHB_PURPLE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPricePurple       = %u", config->GetMaxBidPrice(AHB_PURPLE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPriceOrange       = %u", config->GetMinBidPrice(AHB_ORANGE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPriceOrange       = %u", config->GetMaxBidPrice(AHB_ORANGE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "minBidPriceYellow       = %u", config->GetMinBidPrice(AHB_YELLOW));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxBidPriceYellow       = %u", config->GetMaxBidPrice(AHB_YELLOW));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "manStackGrey            = %u", config->GetMaxStack(AHB_GREY));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxStackWhite           = %u", config->GetMaxStack(AHB_WHITE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxStackGreen           = %u", config->GetMaxStack(AHB_GREEN));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxStackBlue            = %u", config->GetMaxStack(AHB_BLUE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxStackPurple          = %u", config->GetMaxStack(AHB_PURPLE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxStackOrange          = %u", config->GetMaxStack(AHB_ORANGE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "maxStackYellow          = %u", config->GetMaxStack(AHB_YELLOW));
        }
        //AuctionHouseEntry const* ahEntry =  sAuctionMgr->GetAuctionHouseEntry(config->GetAHFID());
        AuctionHouseObject* auctionHouse =  sAuctionMgr->GetAuctionsMap(config->GetAHFID());
        ...
        if (debug_Out)
        {
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Current Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetString().c_str());
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Grey Trade Goods\t%u\tGrey Items\t%u", config->GetItemCounts(AHB_GREY_TG), config->GetItemCounts(AHB_GREY_I)); 
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "White Trade Goods\t%u\tWhite Items\t%u", config->GetItemCounts(AHB_WHITE_TG), config->GetItemCounts(AHB_WHITE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Green Trade Goods\t%u\tGreen Items\t%u", config->GetItemCounts(AHB_GREEN_TG), config->GetItemCounts(AHB_GREEN_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Blue Trade Goods\t%u\tBlue Items\t%u", config->GetItemCounts(AHB_BLUE_TG), config->GetItemCounts(AHB_BLUE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Purple Trade Goods\t%u\tPurple Items\t%u", config->GetItemCounts(AHB_PURPLE_TG), config->GetItemCounts(AHB_PURPLE_I));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Orange Trade Goods\t%u\tOrange Items\t%u", config->GetItemCounts(AHB_ORANGE_TG), config->GetItemCounts(AHB_ORANGE_I));
            TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "Yellow Trade Goods\t%u\tYellow Items\t%u", config->GetItemCounts(AHB_YELLOW_TG), config->GetItemCounts(AHB_YELLOW_I));
        }
    }
    if (AHBBuyer)
    {
        ...
        if (debug_Out)
        {
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPriceGrey          = %u", config->GetBuyerPrice(AHB_GREY));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPriceWhite         = %u", config->GetBuyerPrice(AHB_WHITE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPriceGreen         = %u", config->GetBuyerPrice(AHB_GREEN));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPriceBlue          = %u", config->GetBuyerPrice(AHB_BLUE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPricePurple        = %u", config->GetBuyerPrice(AHB_PURPLE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPriceOrange        = %u", config->GetBuyerPrice(AHB_ORANGE));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerPriceYellow        = %u", config->GetBuyerPrice(AHB_YELLOW));
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerBiddingInterval    = %u", config->GetBiddingInterval());
			TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "buyerBidsPerInterval    = %u", config->GetBidsPerInterval());
        }
    }
    if (debug_Out) TC_LOG_DEBUG(LOG_FILTER_AUCTIONHOUSE, "End Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetString().c_str());
}