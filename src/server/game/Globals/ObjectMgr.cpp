...
void ObjectMgr::LoadFakeItems()
{
    ...
    if (!result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 fake items. DB table `fake_items` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u fake items.", _fakeItemsStore.size());
}
